...
<?php
if (isset($_GET['discType'])) {
    $discType = $_GET['discType'];

    if (isset($_GET['discPercent']) && is_numeric($_GET['discPercent'])) {
        $discPercent = $_GET['discPercent'];

        if (isset($_GET['customerID']) && is_numeric($_GET['customerID'])) {
            $customerID = $_GET['customerID'];

            // Define VAT rate and discount percent for PWD and Senior
            $vatRate = 0.12; // 12% VAT rate
            $pwdSeniorDiscount = 0.20; // 20% discount for PWD and Senior

            // Calculate the total gross amount based on your logic
            $totalGrossAmount = 550.0; // Replace this with your actual calculation

            // Check the discount type and calculate the discount amount accordingly
            if ($discType === 'pwd' || $discType === 'senior') {
                // For PWD and Senior customers
                $discountAmount = $totalGrossAmount * $pwdSeniorDiscount;
            } elseif ($discType === 'regular') {
                // For regular customers, calculate discount based on the percentage
                $discountAmount = $totalGrossAmount * ($discPercent / 100);
            }

            // Calculate VAT for the total gross amount
            $vatAmount = $totalGrossAmount * $vatRate;

            // Calculate the net amount
            $netAmount = $totalGrossAmount - $discountAmount + $vatAmount;
        }
    }
}
?>
...
<body>
    ...
    <div class="container-fluid">
        ...
        <section class="d-xl-flex flex-column justify-content-xl-center align-items-xl-center">
            ...
            $query = "SELECT orderID, QueueNumber FROM orders ORDER BY OrderID LIMIT 100";
            ...
            if (mysqli_num_rows($query_run) > 0) {
                while ($row = mysqli_fetch_assoc($query_run)) {
                    ...
                    $orderID = $row['orderID'];
            ?>
                <button class="btn btn-primary" type="button" style="padding-right: 20px; padding-left: 20px; border-color: var(--bs-black); background: var(--bs-yellow); color: var(--bs-black) ; margin-bottom: 15px;"><strong>#<?php echo $queueNumber; ?></strong></button>
            <?php
                }
            }
            ?>
                        ...
                        <tbody>
                            <?php 
                                $query = "SELECT DISTINCT product.*, order_items.* 
                                            ...
                                            where orders.queueNumber = '$queueNumber'
